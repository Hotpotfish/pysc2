...
class net1(object):
    ...
    def _setup_placeholders_graph(self):
        ...
        # self.agents_local_observation = tf.placeholder("float", shape=[None, config.COOP_AGENTS_OBDIM], name='agents_local_observation')
    ...
    def _observation_encoder_q(self, agents_local_observation, agents_number, scope_name):
        with tf.variable_scope(scope_name):
            ...
            for i in range(agents_number):
                fc1 = slim.fully_connected(agents_local_observation[:, i], 30, scope='full_connected_1' + '_agent_' + str(i))
                encoder.append(fc1)
    ...
    def _bicnet_build_q(self, encoder_outputs, agents_number, scope_name):
        with tf.variable_scope(scope_name):
            ...
            for i in range(agents_number):
                fc1 = slim.fully_connected(bicnet_outputs[i], self.action_dim, activation_fn=tf.nn.softmax, scope='full_connected1' + '_agent_' + str(i))
                outputs.append(fc1)
