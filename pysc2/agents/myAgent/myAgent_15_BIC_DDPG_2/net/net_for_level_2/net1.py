...
class net1(object):
    ...
    def _bicnet_build_a(self, encoder_outputs, agents_number, scope_name):
        with tf.variable_scope(scope_name):
            outputs = []
            lstm_fw_cell = tf.nn.rnn_cell.BasicLSTMCell(50, forget_bias=1.0, name="lstm_fw_cell")
            lstm_bw_cell = tf.nn.rnn_cell.BasicLSTMCell(50, forget_bias=1.0, name="lstm_bw_cell")
    ...
    def _observation_encoder_c(self, state_input, action_input, agents_number, scope_name):
        with tf.variable_scope(scope_name):
            encoder = []
            for i in range(agents_number):
                input_data = tf.concat([state_input, action_input[:, i]], 1)
    ...
    def _bicnet_build_c(self, encoder_outputs, agents_number, scope_name):
        with tf.variable_scope(scope_name):
            ...
            fc3 = slim.fully_connected(outputs, 1, scope='full_connected3')
