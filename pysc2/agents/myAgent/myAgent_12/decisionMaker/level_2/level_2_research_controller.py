...
class level_2_research_controller:
    def __init__(self):
        self.DataShape = (None, config.MAP_SIZE, config.MAP_SIZE, 39)
        self.controller = decision_maker(DQN(config.MU, config.SIGMA, config.LEARING_RATE, len(sa.research_controller), 5, self.DataShape, 'research_controller'))
        self.index =  handcraft_function.find_controller_index(sa.research_controller)
    # 重训练模式 无需读取外部模型
