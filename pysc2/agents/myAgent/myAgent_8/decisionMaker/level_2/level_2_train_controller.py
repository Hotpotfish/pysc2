...
class level_2_train_controller:
    def __init__(self):
        self.DataShape = (None, config.MAP_SIZE, config.MAP_SIZE, 39)
        self.controller = decision_maker(DQN(config.MU, config.SIGMA, config.LEARING_RATE, len(sa.train_controller), 5, self.DataShape, 'train_controller'))
        self.index = handcraft_function.find_controller_index(sa.train_controller)
