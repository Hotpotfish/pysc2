...
class level_2_harvest_controller:
    def __init__(self):
        self.DataShape = (None, config.MAP_SIZE, config.MAP_SIZE, 39)
        self.controller = decision_maker(DQN(config.MU, config.SIGMA, config.LEARING_RATE, len(sa.harvest_controller), 5, self.DataShape, 'harvest_controller'))
        self.index =  handcraft_function.find_controller_index(sa.harvest_controller)
    # 重训练模式 无需读取外部模型
