...
from pysc2.agents.myAgent.myAgent_12.config.config_for_level_2_build_controller import ACTION_DIM, STATE_DIM
...
from pysc2.agents.myAgent.myAgent_12.tools.handcraft_function_for_level_2_build_controller import get_state, get_reward, assembly_action
...
class level_2_build_controller:
    def __init__(self):
        ...
        self.controller = decision_maker(
            DDPG(config.MU, config.SIGMA, config.LEARING_RATE, ACTION_DIM, self.state_data_shape, 'build_controller'))
    ...
    def train_action(self, obs, save_path):
        ...
        if self.controller.previous_action is not None:
            self.controller.previous_reward = get_reward(self.current_obs)
